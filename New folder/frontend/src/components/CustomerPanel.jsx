...
function OpportunityField({ label, value }) {
  return (
    <div className="panel-field">
      <div className="panel-field-label">{label}</div>
...
export default function CustomerPanel({
  ...
  return (
    <aside className="customer-panel">
      ...
      <div className="panel-section">
        <div className="panel-section-title">Active Opportunity</div>
        {selectedOpportunity ? (
          <div className="panel-two-col">
            <OpportunityField label="Property Type" value={selectedOpportunity.propertyType} />
            <OpportunityField label="Listing Type" value={selectedOpportunity.listingType} />
            <OpportunityField
              label="Preferred Location"
              value={
                Array.isArray(selectedOpportunity.preferredLocations)
                  ? selectedOpportunity.preferredLocations.join(", ")
                  : selectedOpportunity.preferredLocations
              }
            />
            <OpportunityField label="Status" value={selectedOpportunity.status} />
            <OpportunityField label="Interest Level" value={selectedOpportunity.interestLevel} />
            <OpportunityField label="Next Action" value={selectedOpportunity.nextSuggestedAction} />
            <OpportunityField label="Created Date" value={formatDateTime(selectedOpportunity.createdAt)} />
            <OpportunityField label="Updated Date" value={formatDateTime(selectedOpportunity.updatedAt)} />
          </div>
        ) : (
          <div className="empty-note">No opportunities yet for this customer.</div>
        )}
      </div>

      <div className="panel-section" style={{ border: "none" }}>
        <div className="panel-section-title">Other Opportunities</div>
        {opportunities.filter((o) => o.id !== selectedOpportunity?.id).length === 0 ? (
          <div className="empty-note">No other opportunities.</div>
        ) : (
          <div className="opportunity-list">
            {opportunities
              .filter((o) => o.id !== selectedOpportunity?.id)
              .map((opp) => (
                <div key={opp.id} className="opportunity-list-item" onClick={() => onSelectOpportunity(opp.id)}>
                  <div className="opportunity-list-item-title">
                    {opp.propertyType || "Opportunity"}
                    {opp.listingType ? ` · ${opp.listingType}` : ""}
                  </div>
                  <div className="opportunity-list-item-meta">
                    {(opp.status || "new")} · {formatDateTime(opp.updatedAt)}
                  </div>
                </div>
              ))}
          </div>
