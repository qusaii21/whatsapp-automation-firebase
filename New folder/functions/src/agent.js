...
const { extractCurrentRequirement } = require("./currentRequirementExtractor");
const {
...
function buildSearchPropertiesTool(excludeIds) {
  return tool(
    async ({
      ...
      // Normalize each free-text filter into the exact canonical enum
      // string stored on property docs before filtering. Without this, a
      // plural ("Villas"), synonym ("workspace" for Office), or partial
      // phrase ("furnished" for "Fully-Furnished") fails plain equality and
      // silently returns zero results even when matching properties exist.
      // If a value doesn't confidently map to a canonical enum, the filter
      // is skipped rather than applied wrong (fail open, not closed).
      if (propertyType) {
        const canonical = normalizePropertyType(propertyType);
        if (canonical) {
          results = results.filter((p) => p.propertyType === canonical);
        }
      }
      if (listingType) {
        const canonical = normalizeListingType(listingType);
        if (canonical) {
          results = results.filter((p) => p.listingType === canonical);
        }
      }
      if (furnishing) {
        const canonical = normalizeFurnishing(furnishing);
        if (canonical) {
          results = results.filter((p) => p.furnishing === canonical);
        }
      }
      if (possessionStatus) {
        const canonical = normalizePossessionStatus(possessionStatus);
        if (canonical) {
          results = results.filter((p) => p.possessionStatus === canonical);
        }
      }
...
const structuredOutputSchema = z.object({
  ...
  listingType: z
    ...
    .nullable()
    .describe(
      "'Sale' or 'Rent' if the user explicitly stated or clearly implied a transaction type " +
        "this turn (e.g. 'looking to buy' -> Sale, 'looking to rent' -> Rent), else null."
    ),
  // NOTE: this is extraction only — the model's own guess is NOT trusted to
  // decide whether a new opportunity should be created. That decision is
  // made deterministically in opportunities.js/resolveActiveOpportunity from
  // the structured fields above (propertyType, listingType, purpose) plus a
  // regex check for explicit restart phrases in the raw message text. This
  // field is kept only for observability/debug logging.
  startsNewOpportunity: z
    .boolean()
    .describe(
      "Your own best guess at whether the user is clearly starting a distinct, separate sales " +
        "opportunity/search from the 'Current active opportunity' described below. Not used to " +
        "make the actual decision — just logged for comparison against the deterministic rule."
    ),
});
...
async function runAgent({
  ...
  // CURRENT REQUIREMENT EXTRACTION — computed here, immediately after intent
  // classification, so it is available to EVERY return path below, including
  // the zero-search-results short-circuit further down. This must NOT be
  // computed later (e.g. alongside the final structured-output call) because
  // the zero-result short-circuit returns before that point is ever reached
  // — a property search that finds nothing is still a fully valid signal
  // that the customer's requirement changed (villa vs apartment, say), and
  // must not be treated as "no requirement stated" just because inventory
  // didn't match. See currentRequirementExtractor.js for why this is a
  // separate, isolated call rather than reusing the main structured-output
  // call's propertyType/listingType/purpose fields.
  let currentRequirement = { propertyType: null, listingType: null, purpose: null };
  if (searchAllowed) {
    const previousAssistantTurn = [...conversationHistory]
      .slice(0, -1)
      .reverse()
    ...
    if (firstResponse.tool_calls && firstResponse.tool_calls.length > 0) {
      ...
      if (Array.isArray(lastSearchResults) && lastSearchResults.length === 0) {
        ...
        return {
          ...
          timeline: null,
          // purpose/propertyType/listingType come from the isolated
          // current-turn extraction computed above, NOT hardcoded null — a
          // search finding zero matching properties is still a fully valid
          // signal that the customer's requirement (e.g. villa vs apartment)
          // changed, and must still be able to trigger a new opportunity.
          purpose: currentRequirement.purpose,
          propertyFound: false,
  ...
  const structuringMessages = [
    new SystemMessage(
      "Given the conversation so far (including the assistant's draft reply at the end), " +
        ...
        "and `listingType` (each only if explicitly stated or clearly implied this turn, else " +
        "null), and `startsNewOpportunity` per its own field description — a best-effort guess " +
  ...
  const final = {
    ...
    // purpose is an opportunity-identity field — see the CURRENT REQUIREMENT
    // OVERRIDE comment above. Sourced from currentRequirement, not the
    // context-anchored structured call.
    purpose: currentRequirement.purpose,
    propertyFound,
    matchedPropertyId,
    ...
    // propertyType/listingType are opportunity-identity fields — see the
    // CURRENT REQUIREMENT OVERRIDE comment above. Sourced from
    // currentRequirement, not the context-anchored structured call.
    propertyType: currentRequirement.propertyType,
    listingType: currentRequirement.listingType,
  ...
  logger.info("agent: final response (post deterministic override)", {
    llmSaid: {
      ...
      // Kept only for comparison against currentRequirement below — this is
      // the context-anchored guess that is NOT used for opportunity
      // decisions anymore.
      contextAnchoredPropertyType: structured.propertyType,
      contextAnchoredListingType: structured.listingType,
      contextAnchoredPurpose: structured.purpose,
    },
    currentRequirement,
    codeOverrode: final,
  });
